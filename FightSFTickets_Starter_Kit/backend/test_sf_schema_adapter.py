...
from services.schema_adapter import SchemaAdapter, TransformationResult
...
class TestSFSchemaAdapter:
    ...
    def setup_class(cls):
        ...
        # Path to SF JSON file
        project_root = Path(__file__).parent.parent
        cls.sf_json_path = project_root / "cities" / "sf.json"
    ...
    def test_sf_configuration_exists(self):
        ...
        assert self.sf_json_path.exists(), (
            f"SF JSON file not found at {self.sf_json_path}"
        )
        ...
        assert self.original_sf_data["city_id"] == "sf", (
            f"Expected city_id='sf', got '{self.original_sf_data.get('city_id')}'"
        )

    def test_schema_adaptation_success(self):
        """Test that SF schema adaptation succeeds."""
        assert self.adaptation_result.success, (
            f"SF schema adaptation failed: {self.adaptation_result.errors}"
        )
        assert len(self.adaptation_result.errors) == 0, (
            f"Adaptation errors: {self.adaptation_result.errors}"
        )

        # Warnings are acceptable (for optional fields with defaults)
        if self.adaptation_result.warnings:
            print(
                f"Note: Schema adaptation warnings: {self.adaptation_result.warnings}"
            )
    ...
    def test_required_fields_present(self):
        ...
        for field in required_fields:
            assert field in transformed, f"Missing required field: {field}"
            assert transformed[field] is not None, f"Required field {field} is None"
    ...
    def test_city_identification(self):
        ...
        assert transformed["city_id"] == "sf", (
            f"Expected city_id='sf', got '{transformed['city_id']}'"
        )
        assert transformed["name"] == "San Francisco", (
            f"Expected name='San Francisco', got '{transformed['name']}'"
        )
        assert transformed["jurisdiction"] == "city", (
            f"Expected jurisdiction='city', got '{transformed['jurisdiction']}'"
        )
        assert transformed["timezone"] == "America/Los_Angeles", (
            f"Expected timezone='America/Los_Angeles', got '{transformed['timezone']}'"
        )
        assert transformed["appeal_deadline_days"] == 21, (
            f"Expected appeal_deadline_days=21, got {transformed['appeal_deadline_days']}"
        )
    ...
    def test_citation_patterns(self):
        ...
        assert len(patterns) >= 4, (
            f"Expected at least 4 citation patterns for SF, got {len(patterns)}"
        )
        ...
        for expected in expected_sections:
            assert expected in section_ids, (
                f"Missing citation pattern for section: {expected}"
            )
        ...
        for pattern in patterns:
            assert "regex" in pattern, f"Citation pattern missing regex: {pattern}"
            assert "section_id" in pattern, (
                f"Citation pattern missing section_id: {pattern}"
            )
            assert "description" in pattern, (
                f"Citation pattern missing description: {pattern}"
            )
            ...
            try:
                ...
                raise AssertionError(f"Invalid regex '{pattern['regex']}': {e}")
    ...
    def test_sections_structure(self):
        ...
        for section_id in expected_sections:
            ...
            assert section["section_id"] == section_id, (
                f"Section ID mismatch: {section['section_id']} != {section_id}"
            )
            assert "name" in section and section["name"], (
                f"Section {section_id} missing name"
            )
            assert "routing_rule" in section, (
                f"Section {section_id} missing routing_rule"
            )
            assert "phone_confirmation_policy" in section, (
                f"Section {section_id} missing phone_confirmation_policy"
            )

            # Check phone confirmation policy structure
            policy = section["phone_confirmation_policy"]
            assert isinstance(policy, dict), (
                f"Phone policy for {section_id} should be dict"
            )
            assert "required" in policy, (
                f"Phone policy for {section_id} missing 'required' field"
            )
    ...
    def test_phone_confirmation_policies(self):
        ...
        assert sfpd_policy["required"] == True, "SFPD should require phone confirmation"
    ...
    def test_address_structures(self):
        ...
        for field in required_address_fields:
            assert field in main_address, f"Main address missing {field}"
            assert main_address[field] and main_address[field].strip(), (
                f"Main address {field} is empty"
            )
        ...
        for section_id in complete_sections:
            ...
            assert address["status"] == "complete", (
                f"{section_id} address should be complete"
            )
            for field in required_address_fields:
                assert field in address, f"{section_id} address missing {field}"
                assert address[field] and address[field].strip(), (
                    f"{section_id} address {field} is empty"
                )
    ...
    def test_file_adaptation(self):
        ...
        assert file_data["city_id"] == "sf", "Adapted file has wrong city_id"
    ...
    def test_city_registry_compatibility(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            temp_output = temp_dir_path / "sf.json"  # Must match city_id

            # Adapt and save
            result = self.adapter.adapt_city_file(self.sf_json_path, temp_output)
            assert result.success, (
                f"File adaptation failed for registry test: {result.errors}"
            )
            ...
            # Verify SF is loaded
            sf_config = registry.get_city_config("sf")
            ...
            test_citations = [
                ("912345678", ("sf", "sfmta")),  # SFMTA citation
                ("SF123456", ("sf", "sfpd")),  # SFPD citation
                ("SFSU12345", ("sf", "sfsu")),  # SFSU citation
            ]

            for citation, expected_match in test_citations:
                match = registry.match_citation(citation)
                assert match == expected_match, (
                    f"Citation '{citation}' matched {match}, expected {expected_match}"
                )
    ...
    def test_backward_compatibility(self):
        ...
        for field in unchanged_fields:
            if field in self.original_sf_data:
                assert transformed[field] == self.original_sf_data[field], (
                    f"Field {field} changed: {transformed[field]} != {self.original_sf_data[field]}"
                )
        ...
        assert original_sections == transformed_sections, (
            f"Sections changed: {transformed_sections} != {original_sections}"
        )
        ...
        assert original_pattern_sections == transformed_pattern_sections, (
            f"Citation pattern sections changed: {transformed_pattern_sections} != {original_pattern_sections}"
        )
...
def run_sf_tests():
    ...
    try:
        ...
        print(f"[SETUP FAILED] Cannot run tests: {e}")
        return False
